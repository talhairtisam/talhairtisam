...
export function TechIconMarquee() {
  ...
  return (
    <div className="relative mb-10 overflow-hidden lg:hidden" aria-hidden={false}>
      ...
      >
        {items.map((item, i) => (
          <div
            key={`${item.slug}-${i}`}
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-border bg-bg-elevated/60"
            title={item.name}
          >
